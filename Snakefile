...
rule all:
    input:
        ...
        "analysis/adata_processed_all_keypoints.h5ad"
...
# repeat the above three rules but using all SIFT keypoints instead of a downsampled set
rule compute_sift_all_keypoints:
    input:
        ET_ratios = "data/4DonorAssay_ET_ratios.csv",
        RASA2KO_titrations = "data/4DonorAssay_RASA2KO_titrations.csv",
    output:
        "analysis/adata_all_keypoints.h5ad"
    log:
        "logs/compute_sift_all_keypoints.log"
    params:
        image_folder = config["image_folder_path"],
        downsample_pct = 1.0,  # hard-code to use all keypoints
        donor_ids = ",".join([str(x) for x in config["donor_ids"]]),
        well_id_rows = ",".join(config["well_id_rows"]),
        well_id_cols = ",".join(config["well_id_cols"]),
        rfp_threshold = config["rfp_threshold"],
...
rule cluster_sift_all_keypoints:
    input:
        "analysis/adata_all_keypoints.h5ad"
    output:
        "analysis/adata_kmeans_all_keypoints.h5ad"
    log:
        "logs/cluster_sift_all_keypoints.log"
    params:
        k_values = ",".join([str(x) for x in config["k_values"]]),
    shell:
        '''
...
rule glcm_and_rfp_stats_all_keypoints:
    input:
        "analysis/adata_kmeans_all_keypoints.h5ad"
    output:
        "analysis/adata_processed_all_keypoints.h5ad"
    log:
        "logs/glcm_and_rfp_stats_all_keypoints.log"
    shell:
        '''
        python scripts/glcm_and_rfp_stats.py \
            --input {input} \
